...
import { Layout, Menu, Form, Cascader, Input, Button, Avatar, message, Modal } from 'antd';
...
const DoctorManagementPage = () => {
    ...
    const fetchDoctors = async () => {
      try {
        ...
        if (response.ok) {
            ...
            phone: doctor.phone,
            avatar: `https://api.dicebear.com/7.x/miniavs/svg?seed=${index + 1}`,
          }));
          setDataSource([...doctors, {
            ...
            office: '',
            phone: '',
            avatar: `https://api.dicebear.com/7.x/miniavs/svg?seed=${doctors.length + 1}`,
          }]);
          setEditableKeys(doctors.map((doctor) => doctor.id).concat(doctors.length + 1));
        } else {
  ...
  const handleSave = async (record) => {
    if (!record.name || !record.department || !record.office || !record.phone) {
    ...
    if (isAddMode) {
      try {
        const response = await fetch('http://localhost:5000/admin/create_doctor', {
          method: 'POST',
          headers: {
        ...
        if (result.id) {
          setDataSource([
            ...
            },
            {
              ...
              office: '',
              phone: '',
              avatar: `https://api.dicebear.com/7.x/miniavs/svg?seed=${dataSource.length + 1}`,
            }
          ]);
      ...
      try {
        console.log(record);
        const response = await fetch(`http://localhost:5000/admin/doctor/${record.employeeId}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`,
  ...
  const handleDelete = async (record) => {
    try {
      ...
      if (response.ok) {
        setDataSource(dataSource.filter(item => item.employeeId !== record.employeeId));
  ...
  const columns = [
    ...
    },
    {
      ...
      renderFormItem: () => (
        <Form.Item>
          <Cascader options={genders.map((gender) => ({ value: gender, label: gender }))} placeholder="请选择性别" />
        </Form.Item>
      ),
    },
    {
      ...
      renderFormItem: () => (
        <Form.Item>
          <Cascader options={titles.map((title) => ({ value: title, label: title }))} placeholder="请选择职称" />
        </Form.Item>
      ),
    },
    {
      ...
      renderFormItem: () => (
        <Form.Item>
          <Cascader options={departments.map((dep) => ({ value: dep, label: dep }))} placeholder="请选择科室" />
        </Form.Item>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, record, index) => {
        if (index === dataSource.length - 1) {
          return [
            <Button type="primary" onClick={() => handleEdit({}, true)}>添加</Button>,
          ];
        }
        return [
              ...
              <Form layout="vertical">
                <Form.Item label="医生工号">
                  <Input
                    ...
                    onChange={(e) => setCurrentRecord({ ...currentRecord, employeeId: e.target.value })}
                  />
                </Form.Item>
                <Form.Item label="姓名">
                  <Input
                ...
                <Form.Item label="性别">
                  <Cascader
                    ...
                    onChange={(value) => setCurrentRecord({ ...currentRecord, gender: value[0] })}
                  />
                </Form.Item>
                <Form.Item label="职称">
                  <Cascader
                    ...
                    onChange={(value) => setCurrentRecord({ ...currentRecord, title: value[0] })}
                  />
                </Form.Item>
                <Form.Item label="科室">
                  <Cascader
                    ...
                    onChange={(value) => setCurrentRecord({ ...currentRecord, department: value[0] })}
                  />
                </Form.Item>
                <Form.Item label="办公室">
                  <Input
                    value={currentRecord.office}
                    onChange={(e) => setCurrentRecord({ ...currentRecord, office: e.target.value })}
                  />
                </Form.Item>
