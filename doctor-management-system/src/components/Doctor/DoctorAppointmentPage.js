...
const DoctorAppointmentPage = () => {
    ...
    const fetchSchedule = async () => {
      try {
        const response = await fetch(
          "http://121.40.44.68:5000/doctor/schedule",
          {
            method: "GET",
            headers: {
        ...
        scheduleData.forEach((item, index) => {
          ...
          quota.push(
            {
              id: index * 2 + 1,
              timeSlot: `${formattedDate} 上午`,
              quota: item.morning_limit,
            },
            {
              id: index * 2 + 2,
              timeSlot: `${formattedDate} 下午`,
              quota: item.afternoon_limit,
            }
          );
        });
        setQuotaData(quota);
  ...
  useEffect(() => {
    ...
    const fetchAppointments = async () => {
      try {
        const response = await fetch(
          "http://121.40.44.68:5000/doctor/appointments",
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    try {
      ...
      updatedData.forEach((item) => {
        ...
        const existingSchedule = schedules.find(
          (schedule) => schedule.date === datePart
        );
      ...
      const response = await fetch("http://121.40.44.68:5000/doctor/schedule", {
        method: "POST",
        headers: {
      ...
      message.error("预约名额设置少于当前预约数量，设置失败");
    }
  };
